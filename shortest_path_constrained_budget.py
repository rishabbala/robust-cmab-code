...
class simulateOnlineData:
    ...
    def runAlgorithms(self, algorithms, oracle_params):
        ...
        for iter_ in range(self.iterations):
            ...
            for alg_name, alg in list(algorithms.items()): 
                ...
                if iter_ % 200 == 0:
                    print(iter_, alg_name, total_cost, Superarm)
    ...
    def resultRecord(self, iter_=None):
        if target_type == "spchosen":
            ...
            self.filenameOptimalRate = os.path.join(save_address, 'Rate{}.csv'.format(target_type + str(self.seed)))
        if target_type == "spchosen2":
            self.filenameWriteCost = os.path.join(save_address, 'Cost{}.csv'.format(target_type + str(self.seed)))
            self.filenameOptimalRate = os.path.join(save_address, 'Rate{}.csv'.format(target_type + str(self.seed)))
        if target_type == "random":
            self.filenameWriteCost = os.path.join(save_address, 'Cost{}.csv'.format(str(self.seed)))
            self.filenameOptimalRate = os.path.join(save_address, 'Rate{}.csv'.format(str(self.seed)))     
        if target_type == "walk":
            self.filenameWriteCost = os.path.join(save_address, 'Cost{}.csv'.format(str(self.seed)))
            self.filenameOptimalRate = os.path.join(save_address, 'Rate{}.csv'.format(str(self.seed)))     
        ...
        if iter_ is None:
            ...
            with open(self.filenameOptimalRate, 'w') as f:
                f.write('Time(Iteration)')
                l = []
                for alg_name in algorithms.keys():
                    if 'Attack' in alg_name:
            ...
            with open(self.filenameOptimalRate, 'a+') as f:
                f.write(str(iter_))
                l = []
                for alg_name in algorithms.keys():
                    if 'Attack' in alg_name:
                        l.append(str(algorithms[alg_name].num_optimal_played[-1]))
                f.write(',' + ','.join(l))
                f.write('\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_address",  type=str, default='./SimulationResults/ShortestPathConstrainBudget')
    ...
    for seed in range(0,num_exp):
        ...
        optimal_path = LCB1Algorithm(P, oracle).decide(oracle_params)
        for o in range(len(optimal_path)-1):
            optimal_path[o] = (optimal_path[o], optimal_path[o+1])
        optimal_path = optimal_path[:-1]
        algorithms['CLCB_Attack'] = LCB1AlgorithmAttack(P, oracle, Target, optimal_path = optimal_path, iter_cutoff = 1000)
